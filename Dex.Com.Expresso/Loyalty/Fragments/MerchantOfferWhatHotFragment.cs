...
{
    public class MerchantOfferWhatHotFragment : BaseFragment
    {
        private ListView mLstView;
        private MerchantProductsItemsAdapter adapter;
        public static MerchantOfferWhatHotFragment NewInstance()
        {
            var frag1 = new MerchantOfferWhatHotFragment { Arguments = new Bundle() };
            return frag1;
        }
        ...
        public override View OnCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState)
        {
            ...
            thread.OnResult += (ServiceResult result) =>
            {
                if (result.Data is List<MerchantProduct>)
                {
                    ...
                    lstItem = lstItem.Where(p => p.decRating >= 4).ToList();
