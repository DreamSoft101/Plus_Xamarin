...
namespace Loyalty.Processing.Connections
{
    public class BaseClient
    {
        ...
        public async Task<string> getData()
        {
            ...
            AddParameter("APIid", Cons.APIid);

            mRstClient = new RestClient(Cons.API_URL + StrMethod);
            
            mRstRequest = new RestRequest("", Method.POST);
            //mRstClient.Timeout = new TimeSpan(;
            string strBody = JsonConvert.SerializeObject(mParameters);
            mRstRequest.AddBody(mParameters);
            ...
            {
                LogUtils.WriteLog(TAG,"URL:"  + Cons.API_URL + StrMethod +  " /Input: " + strBody + "/Error: " + ex.Message + "/Time: " + (DateTime.Now - mdteStart).TotalMilliseconds + " ms");
                return null;
            }
        }
